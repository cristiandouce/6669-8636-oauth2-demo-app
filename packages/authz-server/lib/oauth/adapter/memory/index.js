...
module.exports = class InMemoryOAuthProvider {
  /**
   * Get access token.
   */
  getAccessToken(bearerToken) {
    DebugControl.log.functionName("OAUTH2 MEMORY ADAPTER: getAccessToken");
    DebugControl.log.variable({ name: "input", value: { bearerToken } });

    return OAuthTokensModel.findOne({ accessToken: bearerToken }).then(
      (token) => {
  ...
  getClient(clientId, clientSecret) {
    DebugControl.log.functionName("OAUTH2 MEMORY ADAPTER: getClient");
    ...
    return OAuthClientsModel.findOne(query).then((client) => {
  ...
  getRefreshToken(refreshToken) {
    DebugControl.log.functionName("OAUTH2 MEMORY ADAPTER: getRefreshToken");
    DebugControl.log.variable({
      name: "input",
      value: { refreshToken },
    });

    return OAuthTokensModel.findOne({ refreshToken }).then((token) => {
      DebugControl.log.variable({ name: "output", value: token });
      return token;
    });
  }
  ...
  getUser(username, password) {
    DebugControl.log.functionName("OAUTH2 MEMORY ADAPTER: getUser");
    DebugControl.log.variable({
      name: "input",
  ...
  saveToken(token, client, user) {
    DebugControl.log.functionName("OAUTH2 MEMORY ADAPTER: saveToken");
    DebugControl.log.variable({
      name: "input",
  ...
  saveAuthorizationCode(code, client, user, callback) {
    DebugControl.log.functionName(
      "OAUTH2 MEMORY ADAPTER: saveAuthorizationCode"
    );
    DebugControl.log.variable({
      name: "input",
  ...
  getAuthorizationCode(authorizationCode, callback) {
    DebugControl.log.functionName(
      "OAUTH2 MEMORY ADAPTER: getAuthorizationCode"
    );
    DebugControl.log.variable({
      name: "input",
  ...
  revokeAuthorizationCode(code, callback) {
    DebugControl.log.functionName(
      "OAUTH2 MEMORY ADAPTER: revokeAuthorizationCode"
    );
    DebugControl.log.variable({
      name: "input",
      value: { code },
  ...
  saveState(body) {
    DebugControl.log.functionName("OAUTH2 MEMORY ADAPTER: saveState");
    DebugControl.log.variable({
      name: "input",
      value: { body },
    });
  ...
  getState(state) {
    DebugControl.log.functionName("OAUTH2 MEMORY ADAPTER: getState");
    DebugControl.log.variable({
      name: "input",
      value: { state },
    });
